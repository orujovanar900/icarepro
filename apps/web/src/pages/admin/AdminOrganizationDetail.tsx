...
// ── FIX 1: typed interface replaces `as any[]` ────────────────────────────────
...
function BillingHistoryTab({ orgId }: { orgId: string }) {
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            return res.data.data as BillingHistoryRow[];
        },
    });
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-border text-left text-xs text-muted uppercase tracking-wide">
                        ...
                        <th className="pb-3 pr-4 font-medium">Əvvəlki status</th>
                ...
                <tbody className="divide-y divide-border">
                    {data.map((row: BillingHistoryRow) => (
                        <tr key={row.id} className="hover:bg-surface/50 transition-colors">
                            ...
                            <td className="py-3 pr-4">{row.previousStatus ? <StatusBadge status={row.previousStatus} /> : '—'}</td>
